...
router.get('/PARPRE/ping', (req, res) => {

    exec(`if(test-connection ${req.query.server} -count 1){return $true}`, { 'shell': 'powershell.exe' }, (error, stdout, stderr) => {
...
router.get('/PARPRE/service', (req, res) => {
    exec(`$service = get-service -cn ${req.query.server} -name '${req.query.name}'; return $service.status`, { 'shell': 'powershell.exe' }, (error, stdout, stderr) => {
...
router.get('/PARPRE/database', (req, res) => {
    ...
    exec(`$service = get-service -cn ${req.query.server} -name 'SQL Server Agent(${req.query.database})'; return $service.status`, { 'shell': 'powershell.exe' }, (erro, stdout, stderr) => {
        console.log("out ",stdout)
        console.log("error ",stderr)
        
        stdout === ""
            ? console.log(req.query.database, "n'existe pas sur le server ", req.query.server)
            : console.log("database : ", req.query.databae, "server :", req.query.server, "state : ", stdout.replace(/(\r\n|\n|\r)/gm, ""))
...
router.get('/PARPRE/testPath', (req, res) => {
    ...
    ,{ 'shell': 'powershell.exe' },(error, stdout, stderr) => {
        ...
        res.send({ state: stdout.toLowerCase().replace(/(\r\n|\n|\r)/gm, "") })
