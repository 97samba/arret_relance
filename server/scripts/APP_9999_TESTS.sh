...
Relance_App()
{
		...
		then
				...
				USER=
				CMD="script    sw11204" 
				CMD_WIN="powershell ./script.ps1    sw11204" 
				echo
		...
		then
				...
				SRV=sw11203
				USER=
				CMD="database start instance MSSQL sw11203" 
				CMD_WIN="powershell ./database.ps1 start instance MSSQL sw11203" 
				echo
				echo "DEBUT : $(date +'%d/%m/%Y %H:%M:%S')"
				echo "Serveur "$SRV" - ["$TYPE_ACTION":"$APPLI":"$ETAPE"/"$NB_ETAPE"]" 
				echo "Commande : "$CMD_WIN
		...
		then
				...
				SRV=sw11203
				USER=
				CMD="database status instance MSSQL sw11203" 
				CMD_WIN="powershell ./database.ps1 status instance MSSQL sw11203" 
				echo
				echo "DEBUT : $(date +'%d/%m/%Y %H:%M:%S')"
				echo "Serveur "$SRV" - ["$TYPE_ACTION":"$APPLI":"$ETAPE"/"$NB_ETAPE"]" 
				echo "Commande : "$CMD_WIN
...
Arret_App()
{
		...
		then
				...
				SRV=sw11203
				USER=
				CMD="database stop instance MSSQL sw11203" 
				CMD_WIN="powershell ./database.ps1 stop instance MSSQL sw11203" 
				echo
				echo "DEBUT : $(date +'%d/%m/%Y %H:%M:%S')"
				echo "Serveur "$SRV" - ["$TYPE_ACTION":"$APPLI":"$ETAPE"/"$NB_ETAPE"]" 
				echo "Commande : "$CMD_WIN
